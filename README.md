...
## インストール
1. [リリース](https://github.com/rynan4818/VMCFaceShortcutTransitionMod/releases)からダウンロードした `VMCFaceShortcutTransitionMod.dll` を VMC の `Mods` フォルダに配置します。
...
| `ApplyOnlyWhenSoftChangeFalse` | bool | `true` | `true` の場合、`KeyAction.SoftChange == true` のアクションには遷移を適用しません。現在のVMC(v0.60r1)では未使用の変数 |
| `ActionRules` | array | `[]` | 名称ごとの個別設定。 |

### ActionRules 項目

| キー | 型 | 既定値 | 説明 |
| --- | --- | --- | --- |
| `ActionName` | string | `""` | ルール対象の保存名称。大文字小文字を無視して照合します。 |
| `Enable` | bool | `true` | その名称の遷移ルール有効/無効。`false` の場合は遷移せず VMC 本来の即時反映に戻ります。 |
| `TransitionSec` | float | `0.2` | その名称専用の遷移秒数。`0` 以下は即時反映（補間なし）です。 |
...
## ビルド
1. `VMCFaceShortcutTransitionMod.sln` をVisualStudio2026で開きます。
2. .NET Framework 4.8 クラスライブラリとしてビルドします。
3. NuGet パッケージ（HarmonyX / ILRepack）を復元します。(VisualStudioが自動で復元)
4. ビルド後、ILRepack で必要な依存 DLL を `VMCFaceShortcutTransitionMod.dll` に統合します。(Releaseビルド時に自動統合)
